        ...
        for line in open(file_path, 'r', encoding='utf-8'):
            ...
            if line.startswith('AB'):
                ...
                for idx, str_index in enumerate(result):
                    ...
                    end_index = result[idx+1][0] if idx+1 != len(result) else len(line)
                    name = line[str_index[0]:str_index[1]]
                    if name == 'AB' or name == 'AW':
                        indexs = line[start_index:end_index].lstrip('[').rstrip(']').split('][')
                ...
                for idx, str_index in enumerate(result):
                    start_index = str_index[1]
                    end_index = result[idx+1][0]-1 if idx+1 != len(result) else len(line)-1
                    indexs = line[start_index:end_index].lstrip('[').rstrip(']')
                    add_step["grount_truth"].append([ord(indexs[1])-97, ord(indexs[0])-97])
